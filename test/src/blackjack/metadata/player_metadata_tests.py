...
class PlayerMetadataTestCase(unittest.TestCase):
    ...
    def test_cards_and_score_are_shown(self):
        ...
        for (index, card_json) in enumerate(cards_json):
            self.assertEqual(player.cards[index].suitToString(),
                             card_json[CardMetadata.Keys.suit])
            self.assertEqual(player.cards[index].valToFace(),
                             card_json[CardMetadata.Keys.value])
    ...
    def test_cards_and_score_hidden(self):
        ...
        self.assertEqual(player.cards[1].suitToString(),
                         revealed_card_json[CardMetadata.Keys.suit])
        self.assertEqual(player.cards[1].valToFace(),
                         revealed_card_json[CardMetadata.Keys.value])
