...
class ResultsMetadata:
  ...
  def json_repr_for_win(self):
    return {
      ResultsMetadata.Keys.result: 'Win',
      ResultsMetadata.Keys.winning_player: self.winning_players_metadata[0].json_repr()
    }

  def json_repr_for_tie(self):
    return {
      ResultsMetadata.Keys.result: 'Tie'
    }
  ...
  def from_game(game):
    ...
    winning_players_metadata = [PlayerMetadata.from_player(player) for player in winning_players]
