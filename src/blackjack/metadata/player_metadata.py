...
class PlayerMetadata:
  ...
  def json_repr(self):
    json = {
      PlayerMetadata.Keys.name: self.name,
      PlayerMetadata.Keys.their_turn: self.is_their_turn,
      PlayerMetadata.Keys.cards: [metadata.json_repr() for metadata in self.card_metadata]
    }
    if self.show_score:
